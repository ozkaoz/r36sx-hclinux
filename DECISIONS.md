...
## ADR-015 — USB networking de producción = NCM; overlay azul del AVP aceptado como limitación display-only

- **DATE:** 2026-09-24
- **STATUS:** ACTIVE
- **SCOPE:** Fase 9-6e — transporte de red USB de producción (stack TreeFrogUI) y estado documental de la plataforma.
- **CONTEXT:** el firmware AVP de fábrica pinta un overlay azul uniforme (`06 f2`, stateful — persiste tras B hasta reboot) cuando hay networking activo. Vías de evitación probadas y REFUTADAS físicamente: (1) DTS `/hcrtos/usb0 status=disabled` (v15, kernel `fd4f0d0e` + dtb `116ddf26`) — el azul persiste; (2) subclase CDC — NCM/RNDIS azules, ECM compilado sin probar (premisa muerta); (3) PPP sobre CDC-ACM (kernel `e07844bd`, `net_ppp.sh`) — azul TAMBIÉN, incluso con LCP sin respuesta del PC. El serial ACM puro (shell interactivo, sin pppd) NO lo dispara. El disparador exacto interno del AVP no es aislable sin ingeniería del firmware AVP.
- **DECISION:** (1) **NCM es el transporte de red de producción** (adaptador nativo Windows + telnet root; networking PHYSICAL PASS bajo el azul). (2) El overlay azul se documenta como **limitación conocida y aceptada**, display-only (kernel/red/shell vivos debajo). (3) Transportes alternativos (ECM/RNDIS/PPP/SLIP/serial) quedan como experimentales/laboratorio. (4) La eliminación del azul solo es viable vía **firmware AVP propio** (workspace `~/work/r36sx-hclinux/avp-build/`) — clase D, PARCADO, requiere autorización hardware.
- **RATIONALE:** dos fixes plausibles refutados con evidencia física (`docs/experiments/2026-09-24_9-6e-ppp-slip.md`); la red funciona bajo el azul; el valor del acceso remoto (telnet root) supera el costo visual durante la sesión de red.
