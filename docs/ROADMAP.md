...
| **9-6c** | **Kernel module loader** | **CAUSA RAÍZ CAZADA (2026-09-25)**: gcc Codescape 6.3.0 miscompila la división constante /12 (`sra>>2`+`mul` low-32 sin `mfhi`); los hcdrivers vendor inyectan ksymtab entries legacy de 8 B → `__ksymtab` % 12 == 8 → nmemb=0x55556348 (verificación matemática vs registros del OOPS) → bsearch fuera del array → OOPS determinista en cmp_name. Fix **0005/9005** (punteros como enteros + `divu` tras asm-barrier). Kernel #44 `e9d95de9` BUILD PASS + gates — **deploy+PHYSICAL TEST pendiente** | 🔧 FIX LISTO |
